...
export function SendInterpreterInquiryButton({ jobId, sourceLang, targetLang }: Props) {
  ...
  useEffect(() => {
    if (!open || interpreters.length > 0) return
    setFetching(true)
    fetch('/api/admin/translators')
      .then((r) => r.json())
      .then((data) => {
        const list: Interpreter[] = (data.translators ?? []).filter(
          (t: any) => t.is_active !== false && ['interpreter', 'both'].includes(t.vendor_type ?? '')
        )
        ...
        setInterpreters(enriched)
        setSelected(new Set(enriched.filter((t) => t.isMatch).map((t) => t.id)))
      })
      .finally(() => setFetching(false))
  }, [open, sourceLang, targetLang, interpreters.length])
      ...
      {open && (
        <div className="border border-gray-200 rounded-lg p-4 bg-white shadow-sm w-full max-w-sm">
          <p className="text-xs text-gray-500 mb-3">
            Select interpreters to email with job details. They will respond via a link confirming availability and rate.
          </p>

          {fetching && <Loader2 className="h-4 w-4 animate-spin text-gray-400 mx-auto" />}

          {!fetching && interpreters.length === 0 && (
            <p className="text-xs text-gray-400">No active interpreters found. Make sure vendors have vendor type set to &quot;Interpreter&quot; or &quot;Both&quot;.</p>
          )}
