...
export default function ClientLoginPage() {
  ...
  async function handleEmailLogin(e: React.FormEvent) {
    ...
    const { error: otpError } = await supabase.auth.signInWithOtp({
      email,
      options: {
        emailRedirectTo: `${window.location.origin}/auth/callback?next=/client/jobs`,
      },
    })
    setLoading(false)
    if (otpError) {
  ...
  async function signInWithGoogle() {
    ...
    await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: {
        redirectTo: `${window.location.origin}/auth/callback?next=/client/jobs`,
      },
    })
  }
          ...
          {sent ? (
            <div className="text-center">
              ...
              <p className="text-sm text-gray-500">
                ...
                Click the link to access your orders.
              </p>
              <button
            ...
            <>
              ...
              <Button onClick={signInWithGoogle} className="w-full" variant="outline" disabled={googleLoading}>
                {googleLoading ? (
